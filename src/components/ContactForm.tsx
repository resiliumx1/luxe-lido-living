...
const queryNeedMap: Record<string, string> = {
  "traditional-construction": "build-home",
  "container-builds": "container-modular",
  "insulated-panel-builds": "build-home",
  "renovations-trades": "renovate-repair",
  "property-sales-land": "buy-property",
...
export default function ContactForm({ dark = false }: { dark?: boolean }) {
  ...
  const renderStepOne = () => (
    <div className="space-y-6">
      ...
      {form.need === "renovate-repair" && <div><label className={`text-label block mb-3 ${labelColor}`}>Which trade do you need? <span className="text-primary">*</span></label><OptionSelector name="trade" options={TRADES.map((trade) => ({ value: trade.toLowerCase().replaceAll(" ", "-"), label: trade }))} value={form.trade} onChange={(trade) => setForm({ ...form, trade })} onDark={dark} /></div>}
